...
def dumb_defence_try():
    while True:
        print('Попробуйте угадать слово или букву')
        actual_try = input()
        for c in actual_try:
            if not c.isalpha():
                print('Здесь не только буквы, ваш ввод должен состоять только из букв')
                continue
        return actual_try.lower()
...
def play(word):
    ...
    while tries != 0:
        actual_try = dumb_defence_try()
        if actual_try in guessed_letters or actual_try in guessed_words:
            ...
            if actual_try == word:
                ...
                if len(actual_try) > 1:
                    guessed_words.append(actual_try)
                    tries -= 1
                    print(
                        f'Не правильный ответ, у вас осталось {tries} попыток')
                    print(display_hangman(tries))
                    show_word(word, guessed_letters)
                    if tries == 0:
                        print(
                            f'Вы не смогли угадать слово: {word}')
                        break
                    continue
                if len(actual_try) == 1:
                    if actual_try in word:
                        guessed_letters.append(actual_try)
                        for c in word:
                        ...
                        tries -= 1
                        print(
                            f'Этой буквы нет в слове, у вас осталось {tries} попыток')
                        print(display_hangman(tries))
                        show_word(word, guessed_letters)
                    if tries == 0:
...
while True:
    ...
    if input().lower() in ['y', 'ye', 'yes', '1', 'yeah', 'yep', 'д', 'да', 'ага']:
        continue
    else:
        break
